...
(function() {
  ...
  bgBtn.addEventListener('click', function () {
    bgCallback = function (url) {
      compressImage(url, 1920, 1080, 0.85, function (compressed) {
        applyBackground(compressed);
        saveCurrentGame();
        if (channel && currentGameId) {
          channel.postMessage({ type: 'background', gameId: currentGameId, url: compressed });
        }
      });
    };
    bgInput.click();
      ...
      var cb = function () {
        if (fileInput.files && fileInput.files[0]) {
          ...
          reader.onload = function (e) {
            compressImage(e.target.result, 400, 400, 0.85, function (compressed) {
              con.image = compressed;
              fill.style.backgroundImage = "url(" + compressed + ")";
              saveCurrentGame();
            });
          };
  ...
  function createGameRow(game) {
    ...
    bgThumb.addEventListener('click', (function (g, thumb) {
      return function () {
        bgCallback = function (url) {
          compressImage(url, 1920, 1080, 0.85, function (compressed) {
            g.background = compressed;
            thumb.style.backgroundImage = 'url(' + compressed + ')';
            var games = loadGames();
            for (var i = 0; i < games.length; i++) {
              if (games[i].id === g.id) { games[i].background = compressed; break; }
            }
            saveGames(games);
          });
        };
        ...
        if (e.data.type === 'play') {
          ...
          for (var gi = 0; gi < gs.length; gi++) {
            if (gs[gi].id === viewerGameId) {
              var saved = gs[gi].contestants;
              if (saved.length !== contestants.length) {
                resumeGame(gs[gi]);
                viewerLock();
              } else {
                for (var ci = 0; ci < saved.length; ci++) {
                  contestants[ci].score = saved[ci].score;
                  contestants[ci].oldScore = saved[ci].oldScore;
                }
              }
              play();
              break;
